...
export function readCodexCliAuth() {
	...
	try {
		...
		const trackedLabel = typeof parsed?.[SHUVQUOTA_LABEL_KEY] === "string"
			? parsed[SHUVQUOTA_LABEL_KEY]
			: typeof parsed?.[LEGACY_CODEX_QUOTA_LABEL_KEY] === "string"
				? parsed[LEGACY_CODEX_QUOTA_LABEL_KEY]
				: null;
		return {
			path,
			exists: true,
...
/**
 * Clear managed label markers when they match the removed account and accountId guard passes.
 * @param {{ label: string, accountId: string }} account
 * @returns {{ updated: boolean, path: string | null, skipped?: boolean, reason?: string }}
 */
export function clearShuvquotaLabelForRemovedAccount(account) {
	...
	delete updatedPayload[SHUVQUOTA_LABEL_KEY];
	delete updatedPayload[LEGACY_CODEX_QUOTA_LABEL_KEY];
	writeFileAtomic(cliAuth.path, JSON.stringify(updatedPayload, null, 2) + "\n", { mode: 0o600 });
	return { updated: true, path: cliAuth.path };
}

/**
 * Guarded migration: promote a managed label marker to activeLabel when accountId matches.
 ...
 */
export function maybeMigrateShuvquotaLabelToActiveLabel(activeStore, cliAuth) {
...
export function detectCodexDivergence(options = {}) {
	...
	const migration = allowMigration
		? maybeMigrateShuvquotaLabelToActiveLabel(activeStore, cliAuth)
 ...
 * Includes tracked label if set by shuvquota switch command
 * @returns {{ accountId: string | null, trackedLabel: string | null, source: "shuvquota" | "native" | null }}
 */
export function getActiveAccountInfo() {
	...
	const source = info.trackedLabel ? "shuvquota" : "native";
...
export async function handleCodexSync(args, flags) {
	...
	try {
		...
		if (!activeLabel) {
			const message = "No activeLabel set. Run 'shuvquota codex switch <label>' first.";
			if (flags.json) {
		...
		if (dryRun) {
			...
			const updatedAuth = {
				...
				[SHUVQUOTA_LABEL_KEY]: activeLabel,
			};
			delete updatedAuth[LEGACY_CODEX_QUOTA_LABEL_KEY];
...
export async function handleClaudeSync(args, flags) {
	...
	try {
		...
		const activeLabel = active.activeLabel ?? null;
		if (!activeLabel) {
			const message = "No activeLabel set. Run 'shuvquota claude switch <label>' first.";
			if (flags.json) {
				console.log(JSON.stringify({ success: false, error: message }, null, 2));
			} else {
				console.error(colorize(`Error: ${message}`, RED));
