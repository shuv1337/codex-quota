...
/**
 * Factory token field map: canonical key → candidate keys in order of preference.
 * Factory uses camelCase (accessToken/refreshToken) from readAuthV2Files.
 */
export const FACTORY_TOKEN_FIELDS = {
	access: ["accessToken", "access_token"],
	refresh: ["refreshToken", "refresh_token"],
	expires: ["expiresAt", "expires_at", "expires"],
	accountId: ["accountId", "account_id"],
};
...
/**
 * xAI SuperGrok / Grok OAuth token field map.
 * Covers pi-style (access/refresh/expires) and hermes-style (access_token/refresh_token).
 */
export const XAI_TOKEN_FIELDS = {
	access: ["access", "access_token", "accessToken"],
	refresh: ["refresh", "refresh_token", "refreshToken"],
	expires: ["expires", "expires_at", "expiresAt"],
	accountId: ["accountId", "account_id", "sub", "principal_id"],
};

/**
 * Google AI Pro / Antigravity OAuth token field map.
 * Covers shuvcode credentials, V1 antigravity-accounts.json, and env blobs.
 */
export const ANTIGRAVITY_TOKEN_FIELDS = {
	access: ["access", "access_token", "accessToken"],
	refresh: ["refresh", "refresh_token", "refreshToken"],
	expires: ["expires", "expires_at", "expiresAt", "expiry_date"],
	projectId: ["projectId", "project_id", "project"],
	email: ["email"],
	paidTier: ["paidTier", "paid_tier"],
};
