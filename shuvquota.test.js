/**
 * Tests for shuvquota.js account loading and utility functions
 * 
 * Run with: bun test
 */
...
const TEST_PI_AUTH_DIR = join(tmpdir(), "shuvquota-pi-auth-" + Date.now());
...
describe("PRIMARY_CMD constant", () => {
	test("equals 'shuvquota'", () => {
		expect(PRIMARY_CMD).toBe("shuvquota");
	});
...
describe("readAuthV2Files", () => {
	const testDir = join(tmpdir(), "shuvquota-crypto-read-" + Date.now());
...
describe("writeAuthV2Files", () => {
	const testDir = join(tmpdir(), "shuvquota-crypto-write-" + Date.now());
...
describe("help output", () => {
	...
	test("main help contains 'shuvquota' as primary command", () => {
		printHelp();
		const output = consoleOutput.join("\n");
		
		// Should show shuvquota in usage examples
		expect(output).toContain("shuvquota");
		// Should show shuvquota as first command in header
		expect(output).toMatch(/^shuvquota/);
	});

	test("all subcommand help contains 'shuvquota'", () => {
		const helpFunctions = [
			printHelpAdd,
			printHelpSwitch,
			printHelpCodexSync,
		...
		for (const helpFn of helpFunctions) {
			...
			// Each subcommand help should contain shuvquota in command examples
			expect(output).toContain("shuvquota");
		}
	});
...
describe("error messages", () => {
	test("do not hardcode codex-usage", () => {
		const source = readFileSync(join(import.meta.dir, "shuvquota.js"), "utf-8");
...
describe("package.json metadata", () => {
	...
	test("has name equal to 'shuvquota'", () => {
		expect(pkg.name).toBe("shuvquota");
	});
	...
	test("files array includes 'shuvquota.js'", () => {
		expect(pkg.files).toContain("shuvquota.js");
	});

	test("files array includes the shuvquota PWA", () => {
		expect(pkg.files).toContain("shuvquota-server.js");
	...
	test("bin includes 'shuvquota' command", () => {
	...
	test("bin includes 'cq' alias", () => {
		...
		expect(pkg.bin["cq"]).toBe("./shuvquota.js");
	});

	test("bin includes 'shuvquota-server' app command", () => {
		expect(pkg.bin).toHaveProperty("shuvquota-server");
		expect(pkg.bin["shuvquota-server"]).toBe("./shuvquota-server.js");
	});
...
describe("README documentation", () => {
	...
	test("uses shuvquota title", () => {
		expect(readme.startsWith("# shuvquota")).toBe(true);
	});

	test("documents npm install -g shuvquota", () => {
		expect(readme).toContain("npm install -g shuvquota");
	});

	test("documents bun add -g shuvquota", () => {
		expect(readme).toContain("bun add -g shuvquota");
	});
...
describe("preflight checks", () => {
	test("fails if name is not shuvquota", () => {
		const result = checkPackageName({ name: "wrong-name" });
		expect(result.pass).toBe(false);
		expect(result.message).toContain("expected 'shuvquota'");
	});
	...
	test("fails if required files are missing", () => {
		...
		expect(result.message).toContain("shuvquota-server.js");
...
describe("loadAccountsFromFile", () => {
	const testDir = join(tmpdir(), "shuvquota-test-" + Date.now());
...
describe("loadClaudeAccountsFromFile", () => {
	const testDir = join(tmpdir(), "shuvquota-claude-test-" + Date.now());
...
describe("loadClaudeOAuthFromClaudeCode", () => {
	const testDir = join(tmpdir(), "shuvquota-claude-oauth-test-" + Date.now());
...
describe("persistClaudeOAuthTokens", () => {
	const testDir = join(tmpdir(), "shuvquota-claude-refresh-" + Date.now());
...
describe("ensureFreshToken", () => {
	const testDir = join(tmpdir(), "shuvquota-openai-refresh-" + Date.now());
...
describe("persistOpenAiOAuthTokens", () => {
	const testDir = join(tmpdir(), "shuvquota-openai-persist-" + Date.now());
	const testAuthFile = join(testDir, "auth.json");
	const opencodeAuthPath = join(testDir, "opencode", "auth.json");
...
describe("handleSwitch", () => {
	...
	test("exits with error when no label provided", async () => {
		...
		expect(consoleOutput.error.join("\n")).toContain("Usage: shuvquota codex switch <label>");
	});
...
describe("handleRemove", () => {
	...
	test("exits with error when no label provided", async () => {
		...
		expect(exitCode).toBe(1);
		expect(consoleOutput.error.join("\n")).toContain("Usage: shuvquota codex remove <label>");
	});
...
describe("activeLabel behavior (codex)", () => {
	...
	test("remove clears activeLabel and managed label markers when accountId matches", async () => {
		...
		expect(updatedCliAuth.shuvquota_label).toBeUndefined();
...
describe("detectCodexDivergence", () => {
	...
	test("migrates the legacy managed label when accountId matches", () => {
	...
	test("does not migrate shuvquota_label when accountId mismatches", () => {
...
describe("handleCodexSync", () => {
	...
	test("sync pushes activeLabel tokens to existing CLI stores", async () => {
		...
		expect(updatedAuth.shuvquota_label).toBe(activeLabel);
		expect(updatedAuth.codex_quota_label).toBeUndefined();
...
describe("--local flag", () => {
	const testDir = join(tmpdir(), "shuvquota-local-flag-" + Date.now());
...
describe("printHelpFactory", () => {
	...
	test("contains shuvquota primary command", () => {
		printHelpFactory();
		const output = consoleOutput.join("\n");
		expect(output).toContain("shuvquota");
	});
...
describe("handleFactoryRemove", () => {
	...
	test("confirmation cancel → no changes", async () => {
		...
		const { promptConfirm: _pc } = await import("./shuvquota.js");
...
describe("persistFactoryTokens", () => {
	const testDir = join(tmpdir(), "shuvquota-factory-persist-" + Date.now());
	const testContainerPath = join(testDir, "factory-accounts.json");
...
describe("ensureFreshFactoryToken", () => {
	const testDir = join(tmpdir(), "shuvquota-factory-refresh-" + Date.now());
	const testContainerPath = join(testDir, "factory-accounts.json");
	const testAuthFile = join(testDir, "factory", "auth.v2.file");
	const testKeyFile = join(testDir, "factory", "auth.v2.key");
	let originalFetch;
...
describe("Cross-area: Barrel re-exports completeness", () => {
	test("all factory-crypto.js exports are re-exported from shuvquota.js", () => {
	...
	test("all factory-accounts.js exports are re-exported from shuvquota.js", () => {
	...
	test("all factory-usage.js exports are re-exported from shuvquota.js", () => {
	...
	test("all factory-tokens.js exports are re-exported from shuvquota.js", () => {
	...
	test("Factory handler exports are re-exported from shuvquota.js", () => {
	...
	test("Factory display exports are re-exported from shuvquota.js", () => {
	...
	test("Factory constants are re-exported from shuvquota.js", () => {
